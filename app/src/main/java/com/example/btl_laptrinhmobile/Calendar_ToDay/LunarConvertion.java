...
public class LunarConvertion
{
    private int day, month, year;
    public LunarConvertion()
    {
        day = 1;
        month = 1;
        year = 1979;
    ...
    private double UniversalToJD(int D, int M, int Y)
    {
        double JD;
    ...
    private int[] UniversalFromJD(double JD) {
    ...
    private int[] LocalFromJD(double JD) {
        return UniversalFromJD(JD + 7 / 24.0);
    }
    private double LocalToJD(int D, int M, int Y) {
    ...
    private int[] LunarMonth11(int Y) {
    ...
    private final double[] SUNLONG_MAJOR = new double[] {
    ...
    private void initLeapYear(int[][] ret) {
    ...
    public void SolarToLunar(int solarDay, int solarMonth, int solarYear)
    {
        int yy = solarYear;
        int[][] ly = LunarYear(solarYear); // Please cache the result of this computation for later use!!!
        int[] month11 = ly[ly.length - 1];
        double jdToday = LocalToJD(solarDay, solarMonth, solarYear);
        double jdMonth11 = LocalToJD(month11[0], month11[1], month11[2]);
        if (jdToday >= jdMonth11) {
            ly = LunarYear(solarYear+1);
            yy = solarYear + 1;
        }
        int i = ly.length - 1;
